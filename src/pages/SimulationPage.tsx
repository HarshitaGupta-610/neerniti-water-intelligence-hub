...
const SimulationPage = () => {
  const { t } = useLanguage();
  const { totalConsumed, totalGroundwater, totalRecharged, replenishmentRatio } = useWaterData();

  // Use live data as base, allow user to tweak
  const baseGwPercent = totalConsumed > 0 ? Math.round((totalGroundwater / totalConsumed) * 100) : 40;
  const [municipal, setMunicipal] = useState(totalConsumed > 0 ? Math.round(((totalConsumed - totalGroundwater) * 0.5 / totalConsumed) * 100) : 40);
  const [recycled, setRecycled] = useState(totalConsumed > 0 ? Math.round(((totalConsumed - totalGroundwater) * 0.3 / totalConsumed) * 100) : 20);
  const [rainwater, setRainwater] = useState(totalConsumed > 0 ? Math.round(((totalConsumed - totalGroundwater) * 0.2 / totalConsumed) * 100) : 15);
  ...
  const results = useMemo(() => {
    const totalWater = totalConsumed || 10000;
  ...
  }, [municipal, recycled, rainwater, rechargeBudget, efficiency, groundwater, totalConsumed]);

  const SliderControl = ({
    label, value, onChange, max = 100, icon: Icon, unit = "%",
  }: {
    label: string; value: number; onChange: (v: number) => void; max?: number;
    icon: React.ElementType; unit?: string;
  }) => (
    <div className="mb-5">
    ...
    <div className="min-h-screen pt-20">
      <section className="section-container">
        <div className="mb-10">
          ...
          <p className="text-xs text-muted-foreground mt-1">{t(`Base data: ${(totalConsumed/1000).toFixed(1)}K KL consumed, ratio ${replenishmentRatio.toFixed(2)}x`, `आधार डेटा: ${(totalConsumed/1000).toFixed(1)}K KL उपभोग, अनुपात ${replenishmentRatio.toFixed(2)}x`)}</p>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 glass-card">
            ...
            <div className="mt-4 p-3 rounded-lg bg-muted/50 text-sm">
              ...
              <span className={`font-bold ${groundwater > 50 ? "text-destructive" : groundwater > 25 ? "text-secondary" : "text-accent"}`}>{groundwater}%</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="stat-card text-center">
                ...
                <div className={`text-3xl font-display font-bold ${results.compliance ? "text-accent" : "text-destructive"}`}>{results.ratio.toFixed(2)}x</div>
              ...
              <div className="stat-card text-center">
                <AlertTriangle className={`w-8 h-8 mx-auto mb-2 ${results.riskLevel === "High" ? "text-destructive" : results.riskLevel === "Medium" ? "text-secondary" : "text-accent"}`} />
                <div className="text-xs text-muted-foreground mb-1">{t("Regulatory Risk", "विनियामक जोखिम")}</div>
                <div className={`text-3xl font-display font-bold ${results.riskLevel === "High" ? "text-destructive" : results.riskLevel === "Medium" ? "text-secondary" : "text-accent"}`}>{riskText}</div>
